...
class Localization(Maze):
    ...
    def update_wall_sensor_reading(self, sensor_reading):
        self.new_wall_sensor_reading = sensor_reading
    ...
    def wall_quadrants_prediction(self, wallrow, sensor_reading):
        ...
        wall_narrowed_list_original = self.remove_repeats(wall_narrowed_list_original)

        wall_narrowed_list = self.remove_repeats (wall_narrowed_list)
    ...
    #            wall_input_to_IR.append([i, wall_narrowed_list[i][j]])
        ...
        wall_narrowed_list_original = self.row_indices (wall_narrowed_list_original)

        wall_prediction = self.row_indices (wall_narrowed_list)
    ...
    def final_quadrant_prediction (self, wall_input_to_IR, ir_sensor_reading):
        ...
        if len(quadrant) > 1:
            ...
            direction = self.determine_direction (quadrant)
    ...
    def additional_movement_localization(self, quadrant, new_wall_sensor_reading, wallrow, IRrow):
        ...
        for i in range (0, len(quadrant)):
            a = self.determine_direction_additional (quadrant[i])
            if (a==2):
    ...
    def localize(self):
        ...
        US_wall_prediction = (self.wall_quadrants_prediction(self.us_wall_config, self.us_sensor_reading))[0]
        ...
        self.localized_location = self.final_quadrant_prediction(
                                (self.wall_quadrants_prediction(self.us_wall_config, self.us_sensor_reading))[1], 
                                self.ir_sensor_reading)

        if self.localized_location is None:
            return "Move Forward"
